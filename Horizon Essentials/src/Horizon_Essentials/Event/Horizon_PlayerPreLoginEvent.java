...
public void PreLogin(PlayerLoginEvent e){
	...
	{
		e.disallow(Result.KICK_BANNED, super.msg.getMessage("시간 밴 메시지").replace("/*nextline*/", "\n").replace("[시간]", DataUtil.IntegerToTime((long)((DataManager.players.get((Bukkit.getOfflinePlayer(e.getPlayer().getName()))).tempBan) - (double)System.currentTimeMillis())/1000)).replace("[»çÀ¯]", DataManager.players.get((Bukkit.getOfflinePlayer(e.getPlayer().getName()))).banCause));
	}
}
}
