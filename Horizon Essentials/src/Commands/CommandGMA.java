			...
			if(sender instanceof Player){
				...
				super.m.sendMessage(sender, "게임모드 어드벤쳐");
					...
					if(Bukkit.getPlayer(args[0]) == null)
						...
						super.m.sendMessage(Bukkit.getPlayer(args[0]), "대상 게임모드 어드벤쳐");
						super.m.sendMessage(sender, "대상 게임모드 어드벤쳐",new DoubleString("[대상]",args[0]));
					}
				}
			}else{
				if(args.length <= 0)
					...
					if(Bukkit.getPlayer(args[0]) == null)
						...
						Bukkit.getPlayer(args[0]).setGameMode(GameMode.ADVENTURE);
						super.m.sendMessage(Bukkit.getPlayer(args[0]), "게임모드 어드벤쳐");
						super.m.sendMessage(sender, "대상 게임모드 어드벤쳐",new DoubleString("[대상]",args[0]));
					}
				}
			}
