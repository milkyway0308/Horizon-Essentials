...
public class CommandIgnoreTpa extends CommandsRegistry implements CommandExecutor{
	

	@Override
	public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
			if(sender instanceof Player){
				if(!PlayerUtil.hasPermission(sender, PermissionType.CommandIgnoretpa))
				{
				...
				if(args.length <= 0)
					...
					if(!DataUtil.isAlphaandNumber(args[0]))
						...
						if(DataManager.players.get(p).tpaIgnore.contains(name))
							...
							DataManager.players.get(p).tpaIgnore.add(name);
							super.m.sendMessage(sender, "플레이어 텔레포트 차단",new DoubleString("[대상]",args[0]));
						}
					}
				}
			}else{
